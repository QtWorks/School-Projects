...
class Analyzer {
     ...
     */
    HashSet<DefinitionModel> GetDefinitions(String word){
        //Get definitions for word
        HashSet<DefinitionModel> result = GetDefinitionsRecursive(word);
        //If there are no definitions, return set with just the default definition
        if(result.isEmpty())
            return new HashSet<DefinitionModel>(Arrays.asList(new DefinitionModel(word)));
        else
            return result;
    }
     ...
     */
    HashSet<DefinitionModel> GetDefinitionsRecursive(String word){
        //If there are definitions in the dictionary, stop the process
        HashSet<DefinitionModel> definitions = Lookup(word);
        ...
        for(RuleModel rule : rules){
            ...
            if(sub.length() > 0){
                //Recursive call
                HashSet<DefinitionModel> subDefs = GetDefinitionsRecursive(sub);
     ...
     */
    private boolean DefContains(HashSet<DefinitionModel> definitions, DefinitionModel d) {
     ...
     * @return
     */
    private HashSet<DefinitionModel> Lookup(String word){
        if(dictionary.containsKey(word))
            return dictionary.get(word).GetDefinitions();
        else
            return new HashSet<DefinitionModel>();
    }
     ...
     * @param path
     */
    void PrintDefinitionsFromFile(String path){
        File file = new File(path);
        try {
            Scanner scanner = new Scanner(file);
            while(scanner.hasNext()){
                String line = scanner.nextLine();
                PrintDefinitions(line.trim());
     ...
     * @param word
     */
    void PrintDefinitions(String word){
        for(DefinitionModel def : GetDefinitionsRecursive(word)){
            printDef(def);
        }
    }
    ...
    /**
     * Prints definition in desired format
     * @param d
     */
    private void printDef(DefinitionModel d) {
        System.out.println(String.format("%s %s ROOT=%s SOURCE=%s",d.getWord(),d.getPartOfSpeech(),d.getRoot(),d.getSource()));
    }
...
class WordModel{
     ...
     */
    private HashSet<DefinitionModel> definitions;

    /**
     * Initializes set of definitions
     */
    WordModel(){
        definitions = new HashSet<DefinitionModel>();
    }

    /**
     * Returns set of definitions for word
     */
    HashSet<DefinitionModel> GetDefinitions(){
        return definitions;
    }
