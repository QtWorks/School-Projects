...
namespace SS
{
    ...
    public abstract class AbstractSpreadsheet
    {
        ...
        /// Adds all cells that need to be recalculated to list "changed"
        /// 
        /// Throws CircularException if "start" is contained in itself as a dependency
        /// 
        /// </summary>
        private void Visit(String start, String name, ISet<String> visited, LinkedList<String> changed)
        {
            //Name of the current cell being examined (recursive)
            visited.Add(name);

            // For each of the direct dependents, get its direct dependents
            foreach (String n in GetDirectDependents(name))
            {
                if (n.Equals(start))
                {
                    //If the original or "parent" cell is depending on itself
                    //there is a circular dependency and an exception must be thrown
                    throw new CircularException();
                }
                else if (!visited.Contains(n))
                {
                    //If n has not been visited yet, (cells are added to the 
                    //visited set at the beginning of the recursive method),
                    //we need to examine it and its "children" as well
                    Visit(start, n, visited, changed);
                }
            }
            //Add current cell to cells that need to be changed
            //As long as there are no circular dependencies, the list
            // "changed" should include all of the "family tree" of the 
            //cell named "start". (i.e. The dependents of the dependents of the
            //dependents, etc. etc. etc.)
